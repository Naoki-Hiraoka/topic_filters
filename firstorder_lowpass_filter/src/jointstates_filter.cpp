...
private:
  ...
  firstorder_lowpass_filter::FirstOrderLowPassFilter<double> position_filter_;
  firstorder_lowpass_filter::FirstOrderLowPassFilter<double> velocity_filter_;
  firstorder_lowpass_filter::FirstOrderLowPassFilter<double> effort_filter_;
};
...
int main(int argc, char** argv){
  ...
  pnh.param("position_cutoff_freq", position_cutoff_freq, 1.0); // 1 hz
  pnh.param("velocity_cutoff_freq", velocity_cutoff_freq, 1.0); // 1 hz
  pnh.param("effort_cutoff_freq", effort_cutoff_freq, 1.0); // 1 hz

  std::map<std::string, std::shared_ptr<JointStateFilter> > jointStateMap;
  ros::Subscriber jointStateSub =
    pnh.subscribe<sensor_msgs::JointState>("input",
  ...
  ros::Publisher jointStateFilteredPub = pnh.advertise<sensor_msgs::JointState>("output", 1000);
